var Promise = (function() {
    ...
    function resolvePromise(promise, x, resolve, reject) {
        ...
        if (promise === x) {  //判断不能自己等自己
            return reject(new TypeError('Chaining cycle detected for promise!'))
        }

        if ((x !== null) && ((typeof x === 'object') || (typeof x === 'function'))) {  
            try {
                then = x.then   // 是否是个promise 是object或者function 且有then属性
                if (typeof then === 'function') { //并且then是个function
                    then.call(x, function rs(y) { //
                        if (thenCalledOrThrow) return   //控制只能返回一种状态 成功后不能失败 失败后状态不能再变为成功
                        thenCalledOrThrow = true
                        return resolvePromise(promise, y, resolve, reject)  //递归 直接返回的值是一个普通值 最外层resolve(x) x是普通值 则结束递归
                    }, function rj(r) {
    ...
    Promise.all = function(promises) {
        return new Promise(function(resolve, reject) {
            ...
            for (var i = 0; i < promiseNum; i++) {
                (function(i) {
                    Promise.resolve(promises[i]).then(function(value) {  //循环执行所有promise 记录成功的次数  与promises.length 相等 则表示全部成功，执行resolve状态 否则执行reject
                        resolvedCounter++
